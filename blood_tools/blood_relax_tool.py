...
def QTSlotExceptionRationalizer(*args):
    ...
    is to silence all exceptions, which makes the debugging process impossible. 
    
    Using this decorator fixes that, but PREVENTS THE FUNCTION FROM BEING
    ABLE TO RETURN A VALUE.
    
    DO NOT WRAP ANY METHODS THAT HAVE A RETURN STATEMENT WITH THIS DECORATOR
    """
...
class ROISelectPlot(QtGui.QWidget):
    ...
    def make_image(self, im, vmin=5, vmax=95):
        # only turn autoscale on when first setting the image so that ROI changes won't tweak the autoscale
        self.im = im
...
class MainWindow(QtGui.QWidget):
    ...
    def init_image_data(self):
        ...
        self.grey_roi_patch = None # matplotlib object generated when the ROI is drawn on the image
        self.color_roi_patch = None # matplotlib object generated when the ROI is drawn on the image
    ...
    def init_gui(self):
        ...
        self.button_save_ROI = QtGui.QPushButton('Save ROIs')
        self.button_load_ROI = QtGui.QPushButton('Load ROIs')
       ...
       # self.uncertainty_checkbox = QtGui.QCheckBox('Fit with uncertainty', self)
    ...
    def save_ROI(self, *e):
        ...
        if self.directory:
            out = QtGui.QFileDialog.getSaveFileName(directory=self.directory,caption='ROI filename')
        else:
    ...
    def change_image(self):
        ...
        if num_images > 0:
            if sender_btn == 'slice_first':
                ...
                if self.t2:
                    ind=np.where(np.argsort(self.prep_times)==self.image_index)[0][0]-1
                    self.image_index = np.argsort(self.prep_times)[ind%num_images]
                else:
            ...
            if self.t2:
                ...
                inversion_times = [att['InversionTime'] for att in self.image_attributes]
...
#        if reply == QtGui.QMessageBox.Yes:
        self.clear_roi()
        if self.directory:
            infilename = QtGui.QFileDialog.getOpenFileName(directory=self.directory,caption='ROI filename')
        else:
    ...
    # todo: rework these into one simpler method (its tricky because they're callbacks!)
    @QTSlotExceptionRationalizer("bool")
    ...
    def choose_dir(self, *event):
        ...
        if out:
            ...
            VB17_prep_times = blood_tools.get_T2_prep_times_VB17(self.dicom_list)
...
#            self.load_roi()
    ...
    def process_data(self, *event):
        ...
        if relaxation_type == 'T1':
            ti, signal, stddev = get_T1_decay_signal(self.image_attributes, self.images, roi_list, self.included_slices)
            ...
            bootstrap_signals = bootstrap_decay_signal(self.image_attributes, self.images, roi_list, self.included_slices, iterations=100)
            inversion_recovery.fit(ti, signal, stddev)
            par_uncertainties = fit_bootstrap(ti, bootstrap_signals, inversion_recovery)
            ...
            temp = np.sqrt((par_uncertainties['aa']/inversion_recovery['aa'].value)**2 + (par_uncertainties['T1']/inversion_recovery['T1'].value)**2)
            T1_corr_unc = np.sqrt(temp**2 + par_uncertainties['T1']**2)
            
            
            resids=(inversion_recovery(ti)-np.array(signal))**2/np.array(signal)**2
            RMSD=100*np.sqrt(np.mean(resids))

            str1=r'$\rm{T_1=%.0f \pm %.0f\ ms}$'%(inversion_recovery['T1'].value, par_uncertainties['T1'])
            str2=r'$\rm{T_1\ (LL\ corr)=%.0f \pm %.0f\ ms}$'%(T1_corr, T1_corr_unc)
           ...
           te, signal, stddev = get_T2_decay_signal(self.dicom_list, self.images, roi_list, self.included_slices)
            # throw an error if no T2 prep times found in the dicom header
           if not len(te):
                error = QtGui.QErrorMessage()
                error.showMessage('Failed to find prep times for this dataset, ensure that this is a T2 series')
                error.exec_()
                return
           ...
           spin_echo = fitting.model('M0*exp(-x/T2)', {'M0':np.max(signal),'T2':150})
           spin_echo.fit(te, signal, stddev)
           axes.plot(fit_x_points, spin_echo(fit_x_points), 'b')
          
           resids=(spin_echo(np.array(te))-np.array(signal))**2/np.array(signal)**2
           RMSD=100*np.sqrt(np.mean(resids))

           bootstrap_signals = bootstrap_decay_signal(self.image_attributes, self.images, roi_list, self.included_slices, iterations=100)
           par_uncertainties = fit_bootstrap(te, bootstrap_signals, spin_echo)
           str1=r'$\rm{T_2=%.0f \pm %.0f\ ms, RMSD=%.1f \%%}$'%(spin_echo['T2'].value, par_uncertainties['T2'], RMSD)         
...
def get_T2_decay_signal(dicom_list, image_list, roi_list, included_slices, log_scale=False):
    ...
    for prep_time, image, roi, slice_included in zip(prep_times, image_list, roi_list, included_slices):
        ...
        if slice_included:
            ...
            std_signal_magnitude_over_ROI = blood_tools.calc_ROI_std(roi, image)
