        ...
        if event.inaxes:
            ...
            if event.button == None and self.line is not None:  # Move line around
                ...
                line = plt.Line2D([self.previous_point[0], x], [
                                  self.previous_point[1], y], color=self.color)
                ax.add_line(line)
...
class ROIcircle(ROI):
    ...
    def get_coords(self):
        ...
        if not self.center or not self.radius:
            raise ValueError(
                "cannot get circle coordinates before it's dimesions are defined")
    ...
    def get_indices(self):
        ...
        if not self.center or not self.radius:
            raise ValueError(
                "cannot get circle indicies before its dimensions are defined")
...
class ROIellipse(ROIcircle):
    ...
    def draw(self, axes, figure, color):
        mycirc = Ellipse(self.center, self.width, self.height,
    ...
    def button_press_callback(self, event):
        ...
        if event.inaxes:
            ...
            if event.button == 1:  # If you press the left button
                if self.circ == None:
                    self.circ = Ellipse(
                        (x, y), 0.5, 0.5, facecolor='none', edgecolor=self.color)
                    self.center = x, y
                    ax.add_artist(self.circ)
                else:
                    self.circ.set_color(self.color)
    ...
    def get_coords(self):
        ...
        if not self.center:
            raise ValueError(
                "cannot get ellipse coordinates before the dimensions are defined")
    ...
    def get_indices(self):
        ...
        if not self.center:
            raise ValueError(
                "Cannot get ellipse indices before the dimensions are defined")
