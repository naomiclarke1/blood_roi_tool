...
def calc_ROI_serr(ROI, images):
    ...
    if isinstance(images, list):
        ...
        for jj in np.arange(len(images)):
            serr[jj] = images[jj][ROI.get_indices()].std(
            ) / np.sqrt(len(ROI.get_indices()[0]))
    else:
        serr = images[ROI.get_indices()].std() / \
            np.sqrt(len(ROI.get_indices()[0]))
    return serr
...
def read_dicoms(foldername, attributes=[]):
    ...
    file_list = [fn for fn in os.listdir(
        foldername) if os.path.isfile(os.path.join(foldername, fn))]
    file_list.sort()
    image_list = []
    dicom_list = []
    attribute_list = []
    for file_name in file_list:
        ...
        try:
            ...
            print("Error, invalid dicom file: {}".format(
                os.path.join(foldername, file_name)))
            continue
...
def img_roi_signal(folders_to_process, attributes=[]):
    ...
    mean_signal_mat = np.zeros(
        [len(folders_to_process), len(rois), max_images])
    serr_signal_mat = np.zeros(
        [len(folders_to_process), len(rois), max_images])
...
def SE_fit_new(te, signal, mean_noise=0, noise_floor='n', noise_factor=2):
    ...
    if noise_floor == 'n':
        spin_echo = fitting.model(
            'M0*exp(-x/T2)', {'M0': signal.max(), 'T2': 100})
        ...
        if stop_index < 2:
            ...
            spin_echo = fitting.model(
                'M0*exp(-x/T2)+a', {'M0': signal.max(), 'T2': -1, 'a': signal.min()})
            # spin_echo.fitpars['method']='leastsq'
            # spin_echo.fit(te,signal)
        else:
            spin_echo.fit(te[0:stop_index], signal[0:stop_index],
                          serr_signal[0:stop_index])

    else:  # noise floor=='y'
        print('explicitly fitting for noise floor')
        spin_echo = fitting.model(
            'M0*exp(-x/T2)+a', {'M0': signal.max(), 'T2': 100, 'a': signal.min()})
...
def T2_cpmg_process(folder_to_process, plot='y'):
    ...
    for jj in np.arange(len(rois) - 2):
        ...
        try:
            spin_echo_fit = SE_fit_new(
                np.array(TEs[0:]), mean_sig[0:], mean_noise, 'n')
            if plot == 'y':
...
def T2_cpmg_bootstrap(folder, N=1000, plot='y'):
    ...
    mean_noise = np.mean([img[rois[-2].get_indices()].mean()
    ...
    for kk, roi in enumerate(rois[0:-2]):
        ...
        for nn in np.arange(N):
            ...
            sig = [img[pixels[0][ind], pixels[1][ind]].mean()
                   for img in image_list]
            try:
                spin_echo_fit = SE_fit_new(tes, np.array(sig), mean_noise, 'n')
...
def T1_ir_bootstrap(folders_to_process, N=1000, plot='n'):
    ...
    are obtained by bootstrapping pixels within each ROI"""
    images = [read_dicoms(folder, ['InversionTime'])
        ...
        if N > 0:
            for nn in np.arange(N):
                for mm, roi in enumerate(rois):
                    ...
                    pixels = roi.get_indices()
                    ind = np.random.randint(npix, size=npix)
                    sig[mm] = TI_images[mm][pixels[0]
                                            [ind], pixels[1][ind]].mean()
                fit = IR_fit(TIs, sig)
...
def make_case_report_csv(T2_dict, T1_dict, filename):
    ...
    Hct_sO2s_unique = np.unique(Hct_sO2s.view(np.dtype(
        (np.void, Hct_sO2s.dtype.itemsize * Hct_sO2s.shape[1])))).view(Hct_sO2s.dtype).reshape(15, 3)
    for jj in np.arange(len(Hct_sO2s_unique)):
        ...
        for kk in np.arange(len(taus)):
            sheet1.write(jj + 1, 4 + kk,
                         T2_value_list[indices[kk]].nominal_value)
    book.save(filename)
    ...
    for jj, T2_dict in enumerate(T2_dict_list):
        ...
        for kk in np.arange(len(Hct_sO2s_unique)):
            ...
            for mm in np.arange(len(taus)):
                sheet1.write(kk + 1, 4 + mm,
                             T2_value_list[indices[mm]].nominal_value)
    book.save(filename)
...
def calc_B0_map(folder, roi_mask):
    ...
    phase_images_unwrapped = [unwrap.unwrap(
        np.ma.masked_array(img, roi_mask_inv)) for img in phase_images]
...
def T1_MOLLI_bootstrap(folder, N=1000):
    ...
    for kk, roi in enumerate(rois[0:-2]):
        ...
        for nn in np.arange(N):
            ind = np.random.randint(npix, size=npix)
            sig = np.array([image[pixels[0][ind], pixels[1][ind]].mean()
                            for image in images])
            fit = IR_fit(TIs, sig)
    ...
    for mm, roi in enumerate(rois[0:-2]):
        ...
        for nn in np.arange(N):
            ...
            for jj in np.arange(len(TIs)):
                ind = np.random.randint(npix, size=npix)
                sig_boot[jj] = images[jj][pixels[0]
                                          [ind], pixels[1][ind]].mean()
...
def plot_T2_contour(esp, model):
    ...
    C = plt.contour(Hct_mat, 1000 / R2_ans, 100 * sO2_mat, np.r_[
                    0:40:20, 30:110:10], vmin=0, vmax=110, linewidths=1.5, linestyles='solid', cmap=matplotlib.cm.cool)
    plt.clabel(C, C.levels[::2], colors='k',
               inline=True, fmt="%0.1f", fontsize=14)
...
def plot_T1_contour(model, metHb=0.005):
    ...
    C = plt.contour(100 * sO2_mat, 1000 / R1_ans, Hct_mat, np.r_[
                    0:0.3:0.05, 0.3:1.1:0.1], vmin=0, vmax=0.95, linewidths=1.5, cmap=matplotlib.cm.jet)
    #C = plt.contour(Hct_mat,1000/R1_ans,sO2_mat,np.arange(-0.5,1.05,0.05),vmin=0,vmax=1.1,linewidths=1.5,cmap=matplotlib.cm.cool)
    plt.clabel(C, C.levels[::2], colors='k',
               inline=True, fmt="%0.2f", fontsize=14)
...
def retrieve_dict_data(T2_dict, esp=0, metHb_threshold=0.02):
    ...
    if (esp == 0):  # retrieve all the data
        ...
        Hcts.append([item[0][1] for item in dict_data if item[1].nominal_value != -
                     1.0 and item[0][2] < metHb_threshold])
        sO2s.append([item[0][0] for item in dict_data if item[1].nominal_value != -
                     1.0 and item[0][2] < metHb_threshold])
        esps.append([0.001 * item[0][3] for item in dict_data if item[1]
                     .nominal_value != -1.0 and item[0][2] < metHb_threshold])
        R2s.append([1000 / item[1].nominal_value for item in dict_data if item[1]
                    .nominal_value != -1.0 and item[0][2] < metHb_threshold])
        R2errs.append([(1000 / item[1]).std_dev for item in dict_data if item[1]
                       .nominal_value != -1.0 and item[0][2] < metHb_threshold])
        metHbs.append([item[0][2] for item in dict_data if item[1].nominal_value != -
                       1.0 and item[0][2] < metHb_threshold])
        pt_nos.append([item[0][4] for item in dict_data if item[1].nominal_value != -
                       1.0 and item[0][2] < metHb_threshold])
        T2s.append([item[1].nominal_value for item in dict_data if item[1]
                    .nominal_value != -1.0 and item[0][2] < metHb_threshold])
        T2errs.append([item[1].std_dev for item in dict_data if item[1]
                       .nominal_value != -1.0 and item[0][2] < metHb_threshold])
    else:
        for key in T2_dict:
...
def retrieve_T1_dict_data(T1_dict, metHb_threshold=0.02):
    ...
    Hcts.append([item[0][1]
                 for item in dict_data if item[0][2] < metHb_threshold])
    sO2s.append([item[0][0]
                 for item in dict_data if item[0][2] < metHb_threshold])
    metHbs.append([item[0][2]
                   for item in dict_data if item[0][2] < metHb_threshold])
    pt_nos.append([item[0][3]
                   for item in dict_data if item[0][2] < metHb_threshold])
    R1s.append(
        [1000 / item[1].nominal_value for item in dict_data if item[0][2] < metHb_threshold])
    R1errs.append(
        [(1000 / item[1]).std_dev for item in dict_data if item[0][2] < metHb_threshold])
    T1s.append(
        [item[1].nominal_value for item in dict_data if item[0][2] < metHb_threshold])
    T1errs.append(
        [item[1].std_dev for item in dict_data if item[0][2] < metHb_threshold])
...
def calc_R2(x, a1, a2, a3, b1, b2, c1):
    R2 = (a1 + a2 * x[0] + a3 * x[0]**2) + (b1 * x[0] + b2 * x[0]
                                            ** 2) * (1 - x[1]) + (c1 * x[0] * (1 - x[0])) * (1 - x[1])**2
    return R2
...
def get_T2_prep_times_VB17(dicom_list):
    ...
    for dicom_hdr in dicom_list:
        ...
        if not prep_time_search:
            ...
            value_list = re.findall(prep_times_value_re, str(out.value))[
                0:num_preps]
...
def get_T2_prep_times_VD13A(dicom_list):
    prep_times_re = 'sWipMemBlock.alFree\[1\][ ]+=[ ]+([.\d]+)'
    prep_times_value_re = 'sWipMemBlock.adFree\[\d+\][ ]+=[ ]+([.\d]+)'
...
def bootstrap_fit(image_list, roi,  model, x, iterations=1000):
    # image_list,TEs=read_dicoms(folder,['EchoTime'])
