...
class model:
    ...
    def __init__(self, expr, pars, name="", namespace={}):
        ...
        self.kwargs = {}
        self.expr = expr
        self.pars = sorted([parameter(p[1], p[0])
                            for p in pars.items()], key=lambda x: x.name)
        self.name = name
    ...
    def fit(self, x, y, dy=None):
        ...
        if self.fitpars['method'] == 'curve_fit':
            ...
            fitter = opt.leastsq
            out = fitter(self.residuals, pars, (x, y, dy),
                         full_output=1, **self.kwargs)
            self.covar = out[1]
        else:
            fitter = opt.minimize
    ...
    def uncertainties(self):
        ...
        assert hasattr(
            self, 'covar'), "Only works after fit with curve_fit or leastsq"
